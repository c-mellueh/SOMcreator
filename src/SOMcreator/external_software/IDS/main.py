...
def _build_info(proj: classes.Project, xml_parent: Element) -> None:
    xml_element = SubElement(xml_parent, ids_xsd.INFO, nsmap=NSMAP)
    SubElement(xml_element, ids_xsd.TITLE, nsmap=NSMAP).text = f"Prüfregeln für Projekt '{proj.name}'"
    SubElement(xml_element, ids_xsd.COPYRIGHT, nsmap=NSMAP).text = "Nö"
    ...
    SubElement(xml_element, ids_xsd.AUTHOR, nsmap=NSMAP).text = f"{proj.author}"
...
def _build_attribute_requirement(attribute: classes.Attribute, xml_parent: Element) -> None:
    ...
    if attribute.value_type == value_constants.LIST:
        for value in attribute.value:
            SubElement(xml_restriction, xml_xsd.ENUMERATION, nsmap=NSMAP).set(ids_xsd.VALUE, value)
    ...
    if attribute.value_type == value_constants.RANGE:
        ...
        SubElement(xml_restriction, xml_xsd.MININCLUSIVE, nsmap=NSMAP).set(ids_xsd.VALUE, min_value)
        SubElement(xml_restriction, xml_xsd.MAXINCLUSIVE, nsmap=NSMAP).set(ids_xsd.VALUE, max_value)

    if attribute.value_type == value_constants.FORMAT:
        pattern = "|".join(attribute.value)
        SubElement(xml_restriction, xml_xsd.PATTERN, nsmap=NSMAP).set(ids_xsd.VALUE, pattern)
...
def _build_specification(obj: classes.Object, property_set_dict: dict[classes.PropertySet, list[classes.Attribute]],
    ...
    xml_specification.set(ids_xsd.ATTR_NAME, f"Prüfregel {obj.name} ({obj.ident_value})")
    xml_specification.set(ids_xsd.ATTR_IFCVERSION, ids_xsd.VAL_IFC4)
    xml_specification.set(ids_xsd.ATTR_DESCRIPTION, "Automatisch generierte Attributprüfregel")
